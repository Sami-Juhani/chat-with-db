...
class RagSqlAgent:
    ...
    def _merge_rag_and_sql_reponses(self, question: str, user_orders: str, rag_response: str, sql_response: str) -> str:
        ...
        """
        template = """
        Combine the RAG response and the SQL response into a single, coherent answer. If either response contains "I don't know" or not relevant information for the question, ignore it and use only the valid information. If the user queries about their orders, only include information about their orders by referencing the user's order data. Ensure the response is naturally structured without explicitly stating that data was merged and valid.
