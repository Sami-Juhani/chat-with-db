...
class RagSqlAgent:
    ...
    async def _ask_use_sql_db(self, question: str):
        """
        Uses the SQL database to retrieve the response to the question.

        Args:
            question (str): The question to be asked.

        Returns:
            str: The response generated by the SQL database.
        """
    ...
    def _merge_rag_and_sql_reponses(self, question: str, user_orders: str, rag_response: str, sql_response: str) -> str:
        """
        ...
        Parameters:
            ...
            user_orders (dict): The user's order data.
        ...
        """
        template = """
        Construct a clear and relevant response by combining the RAG and SQL responses while ensuring only necessary information is included. Ignore any response that contains "I don't know" or does not provide relevant details.

        - If the user specifically asks about their orders, include only their orders from the database.
        - If the user asks about orders in general (without specifying their own), include all available orders from the database.
        - If the user does not ask about orders, exclude order-related information entirely.

        Generate a naturally structured response without explicitly stating that data was merged.
        ...
        response = chain.invoke(
            {"question": question, "user_orders": user_orders, "rag_response": rag_response, "sql_response": sql_response})
    ...
    async def ask(self, user, question: str) -> str:
        ...
        if user['type'] == "ADMIN":
            ...
            merged_response = self._merge_rag_and_sql_reponses(
                question, user['orders'], rag_response, sql_response)
