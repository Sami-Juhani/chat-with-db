...
class RagSqlAgent:
    ...
    def _merge_rag_and_sql_reponses(self, question: str, rag_response: str, sql_response: str) -> str:
        """
        ...
        """
        template = f"""
        Combine the RAG response and the SQL response into a single, coherent answer. If either response contains "I don't know" or not relevant information for the question, ignore it and use only the valid information. Ensure the response is naturally structured without explicitly stating that data was merged and valid.
        ...
        response = chain.invoke(
            {"question": question, "rag_response": rag_response, "sql_response": sql_response})
    ...
    async def ask(self, user, question: str) -> str:
        ...
        if user['type'] == "ADMIN":
            ...
            merged_response = self._merge_rag_and_sql_reponses(
                question, rag_response, sql_response)
